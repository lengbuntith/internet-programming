<template>
  <div>
    <dialog :id="`ProEditDialog${Pro._id}`" class="px-20 py-20 dialog-pro-edit">
      <form method="dialog" :id="`ProEditForm${Pro._id}`">
        <div>
          ...
          >
            <h4>Prices</h4>
            <button @click="addNewPrice" type="button">Add</button>
          </div>
          <table style="margin-bottom: 20px">
            ...
            <tr v-for="p in Pro.prices" :key="p._id">
              <td style="text-align: left">{{ p.price }}</td>
              <td style="text-align: left">{{ p.source }}</td>
              <td>
                <div style="display: flex; justify-content: start">
                  ...
                  <button @click="deletePriceByID(p._id)" type="button">
                    X
                  </button>
                </div>
              </td>
        ...
        <div>
          <button style="margin-right: 10px" @click="close" type="reset">
            Cancel
          </button>
...
export default {
  ...
  methods: {
    ...
    addNewPrice() {
      ...
      dialog.dataset.productId = this.Pro._id;
      dialog.showModal();
    },

    editPriceByID(priceInfo) {
      const dialog = document.getElementById("PriceEditDialog");
      dialog.dataset.id = priceInfo.id;
      dialog.childNodes[0][0].value = priceInfo.price;
      dialog.childNodes[0][1].value = priceInfo.source;
      dialog.showModal();
    },

    async deletePriceByID(priceId) {
      await this.$axios.post(`/prices/delete/${priceId}`);
      this.$emit("ProEdited");
    },
  },
};
